...
<meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src https://fonts.gstatic.com data:; img-src 'self' data: blob:; frame-src https://www.google.com; connect-src 'self'; object-src 'none'; base-uri 'self';">
...
describe("build wiring contract", () => {
  ...
  it("source index.html CSP is local-only (no external script origins beyond self/fonts/google)", () => {
    const html = readFileSync(resolve(root, "index.html"), "utf8");
